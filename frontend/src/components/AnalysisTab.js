...
export function renderAnalysisTab(container, initialData = null) {
    ...
    <div class="space-y-6">
      ...
      <div class="card card-glow-cyan" style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap: 1rem;">
        ...
        <div style="display: flex; gap: 0.5rem; align-items: center; flex-wrap: wrap;">
          <button id="btn-reset-analysis-module" class="btn btn-secondary module-reset-btn" type="button" style="font-size: 0.78rem; padding: 0.35rem 0.75rem; border-color: var(--accent-amber); color: var(--accent-amber); display: inline-flex; align-items: center; gap: 0.35rem;" title="Reiniciar este módulo para probar otra vez">
      ...
      <div id="analysis-results-section" style="display: none; flex-direction: column; gap: 1.5rem;">
        ...
        <div class="card card-glow-purple">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 1rem; flex-wrap: wrap; gap: 0.75rem;">
            ...
            <div style="display: flex; gap: 0.5rem; align-items: center; flex-wrap: wrap;">
              ...
              <div style="display: flex; flex-direction: column; gap: 0.25rem;">
                <label style="font-size: 0.75rem; color: var(--accent-rose); font-weight: 700; text-transform: uppercase;">Plano de Bit:</label>
                <select id="bitplane-index-select" style="padding: 0.55rem 0.9rem; font-size: 1.1rem; min-height: 44px; width: auto; border: 1px solid var(--accent-rose); background: rgba(16, 22, 34, 0.95); color: #fca5a5; font-weight: 700; border-radius: 8px;">
                  <option value="0" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;" selected>Bit 0 (LSB - Esteganografía)</option>
                  <option value="1" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">Bit 1 (±2)</option>
                  <option value="2" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">Bit 2 (±4)</option>
                  <option value="3" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">Bit 3 (±8)</option>
                  <option value="4" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">Bit 4 (±16)</option>
                  <option value="5" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">Bit 5 (±32)</option>
                  <option value="6" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">Bit 6 (±64)</option>
                  <option value="7" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">Bit 7 (MSB - Glitches / ±128)</option>
                </select>
              </div>

              <!-- Modo de Vista -->
              <div style="display: flex; flex-direction: column; gap: 0.25rem;">
                <label style="font-size: 0.75rem; color: var(--accent-cyan); font-weight: 700; text-transform: uppercase;">Modo de Vista:</label>
                <select id="bitplane-view-select" style="padding: 0.55rem 0.9rem; font-size: 1.1rem; min-height: 44px; width: auto; border: 1px solid var(--accent-cyan); border-radius: 8px; background: rgba(16, 22, 34, 0.95); color: var(--text-primary); font-weight: 600;">
                  <option value="microscope" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;" selected>🔬 Microscopio Digital (Zoom 16x)</option>
                  <option value="highlight" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">🌐 Resaltador de Zona Inyectada</option>
                  <option value="full" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">▦ Plano de Bits Completo (Matriz 1:1)</option>
                </select>
              </div>

              <!-- Canal -->
              <div style="display: flex; flex-direction: column; gap: 0.25rem;">
                <label style="font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; font-weight: 600;">Canal:</label>
                <select id="bitplane-channel-select" style="padding: 0.55rem 0.9rem; font-size: 1.1rem; min-height: 44px; width: auto; border: 1px solid var(--border-color); border-radius: 8px; background: rgba(16, 22, 34, 0.95); color: var(--text-primary); font-weight: 600;">
                  <option value="all" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">Luminancia (RGB)</option>
                  <option value="red" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">Canal Rojo (R)</option>
                  <option value="green" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">Canal Verde (G)</option>
                  <option value="blue" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">Canal Azul (B)</option>
                </select>
              </div>

              <!-- Paleta de Color -->
              <div style="display: flex; flex-direction: column; gap: 0.25rem;">
                <label style="font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; font-weight: 600;">Paleta de Color:</label>
                <select id="bitplane-color-select" style="padding: 0.55rem 0.9rem; font-size: 1.1rem; min-height: 44px; width: auto; border: 1px solid var(--border-color); border-radius: 8px; background: rgba(16, 22, 34, 0.95); color: var(--text-primary); font-weight: 600;">
                  <option value="neon" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;" selected>⚡ Neón Cyberpunk (Cian / Púrpura)</option>
                  <option value="matrix" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">📟 Terminal Matrix (Verde Fósforo)</option>
                  <option value="amber" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">🟡 Ámbar Radar Forense</option>
                  <option value="heatmap" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">🔥 Termografía (Heatmap)</option>
                  <option value="bw" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">⬛⬜ Blanco y Negro Puro</option>
                </select>
              </div>

              <!-- Modo de Figura / Marcador -->
              <div style="display: flex; flex-direction: column; gap: 0.25rem;">
                <label style="font-size: 0.75rem; color: var(--text-muted); text-transform: uppercase; font-weight: 600;">Figura / Marcador:</label>
                <select id="bitplane-shape-select" style="padding: 0.55rem 0.9rem; font-size: 1.1rem; min-height: 44px; width: auto; border: 1px solid var(--border-color); border-radius: 8px; background: rgba(16, 22, 34, 0.95); color: var(--text-primary); font-weight: 600;">
                  <option value="dots" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;" selected>🟢 Puntos Luminosos Neón (Glow)</option>
                  <option value="crosses" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">✖️ Cruces Periciales (+)</option>
                  <option value="bounding-box" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">🔲 Bloques con Bits (1 / 0)</option>
                  <option value="pixels" style="font-size: 1.25rem; padding: 12px 16px; background-color: #0d1424; color: #f8fafc;">▦ Matriz de Píxeles Pura</option>
                </select>
              </div>
          ...
          <div id="bitplane-preview-container" style="min-height: 460px; max-height: 650px; background: #060a12; border: 1px solid rgba(0, 240, 255, 0.4); border-radius: 8px; overflow: auto; padding: 0.75rem; display: flex; justify-content: center; align-items: center;">
          ...
          <div id="bitplane-kpi-panel" style="margin-top: 1.25rem; border-top: 1px solid var(--border-color); padding-top: 1.25rem;">
            <h4 style="font-size: 1.25rem; color: var(--accent-cyan); margin-bottom: 1rem; display: flex; align-items: center; gap: 0.5rem; font-weight: 700;">
              <span><span class="micon" aria-hidden="true" style="font-size: 1.4rem;">bar_chart</span> </span> Auditoría Cuantitativa de Bits Modificados
            </h4>
  ...
  // ELEMENTOS DOM
  // ==========================================
  const btnResetAnalysisModule = container.querySelector('#btn-reset-analysis-module');
  if (btnResetAnalysisModule) {
    btnResetAnalysisModule.addEventListener('click', () => {
      window.dispatchEvent(new CustomEvent('cyberlab-reset-module', { detail: { module: 'analysis' } }));
    });
  }
    ...
    try {
      ...
      if (conversionNotice) {
        if (conversion.wasConverted) {
          ...
          conversionNotice.innerHTML = `<span class="micon" aria-hidden="true">bolt</span>  <strong>Imagen optimizada:</strong> de <code>${origFormat}</code> a <code>PNG</code> (${conversion.width}×${conversion.height} px) para análisis forense pericial sin sobrecarga.`;
        } else {
  ...
  if (initialData) {
    (async () => {
      try {
        await handleSelectedFile(initialData);
        if (currentAnalysisBlob) {
          await runForensicAnalysis();
        }
      } catch (e) {
        console.error('Error inicializando imagen en análisis forense:', e);
      }
    })();
  }
