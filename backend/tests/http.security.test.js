...
tests.push(['Las rutas costosas agotan su cupo y responden 429', async () => {
  // El cupo estricto de esta prueba es 5 por ventana. Se agota con peticiones
  // invalidas, que son baratas pero cuentan igual para el limitador.
  const statuses = [];
  for (let i = 0; i < 9; i++) {
    const res = await fetch(`${BASE}/api/crypto/encrypt`, {
      method: 'POST',
  ...
  assertTrue(body.error.includes('Demasiadas'), `mensaje inesperado: ${body.error}`);
}]);
...
tests.push(['El health check reporta entorno y estado del pool', async () => {
  const body = await (await fetch(`${BASE}/api/health`)).json();

  assertEquals(body.environment, 'test', 'entorno');
  assertClose(body.concurrency.forensicsWorkers.size, 1, 0);
  assertTrue(body.standards.forensics.includes('RS Analysis'), 'falta el detalle de estandares');
}]);
