...
tests.push(['Una contrasena incorrecta falla por integridad, no por relleno', async () => {
  const error = await assertRejects(
    () => decryptAESGCM(reference.packedBuffer, 'contrasena equivocada'),
    'una contrasena equivocada consiguio descifrar'
  );
  assertTrue(
    error.message.includes('INTEGRIDAD') || error.message.includes('AUTENTICACION'),
    `esperaba un error de integridad, obtuve: ${error.message}`
  );
}]);
