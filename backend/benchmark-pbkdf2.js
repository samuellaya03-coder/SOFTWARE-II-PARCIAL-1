...
async function simularAtaqueFuerzaBruta(iteraciones) {
  ...
  // Cap de seguridad para pruebas interactivas en terminal (evita congelar Node con >1M iteraciones)
  const iterAtaque = Math.min(iteraciones, 1_000_000);
  ...
  for (let i = 0; i < candidatos.length; i++) {
    ...
    await sleep(250); // Pausa visual para ver qué clave se está probando
    
    const t0 = performance.now();
    crypto.pbkdf2Sync(pass, SALT, iterAtaque, 32, 'sha512');
    ...
    await sleep(400); // Pausa visible para que el jurado aprecie cada intento
  }
